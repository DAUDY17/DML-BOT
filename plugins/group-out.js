...
async (conn, mek, m, {
    ...
    if (!q) return reply("❌ Please provide a country code. Example: .out 255");

    const countryCode = q.trim();
    if (!/^\d+$/.test(countryCode)) {
        return reply("❌ Invalid country code. Please provide only numbers (e.g., 255 for +255 Tz numbers)");
    }
